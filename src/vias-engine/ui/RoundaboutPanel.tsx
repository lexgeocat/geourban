  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', fontSize: '0.72rem' }}>
      ...
      >
        ...
        <label style={{ display: 'block', fontSize: '0.65rem', color: 'var(--cad-text-dim)' }}>
          Radio al eje (m)
        </label>
        <input
          type="number"
          ...
          onChange={(e) => setDefaultRadius(parseFloat(e.target.value) || defaultRadiusM)}
          className="cad-input"
        />
        ...
        <select
          value={defaultSides}
          onChange={(e) => setDefaultSides(parseInt(e.target.value, 10))}
          className="cad-input"
        ...
        <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
          <div style={{ flex: 1 }}>
            ...
            <input
              ...
              min={1}
              step={0.5}
              value={defaultRoadWidthM}
              onChange={(e) => setDefaultRoadWidth(parseFloat(e.target.value) || defaultRoadWidthM)}
              className="cad-input"
            />
          </div>
          <div style={{ flex: 1 }}>
            ...
            <input
              ...
              min={0}
              step={0.5}
              value={defaultSidewalkWidthM}
              onChange={(e) => setDefaultSidewalkWidth(parseFloat(e.target.value) || 0)}
              className="cad-input"
            />
          </div>
        </div>
        ...
        <button
          ...
          style={{
            ...
            marginTop: 8,
            height: 28,
            borderColor: mode === 'roundabout' ? 'var(--cad-accent)' : undefined,
            color: mode === 'roundabout' ? 'var(--cad-accent)' : undefined,
          }}
        >
      ...
      {roundabouts.length === 0 ? (
        <p style={{ fontSize: '0.68rem', color: 'var(--cad-text-muted)' }}>
          Todavía no hay rotondas trazadas.
        </p>
      ) : (
        roundabouts.map((rb) => (
          <div
            ...
            style={{
              borderTop: `1px solid ${selectedIds.has(rb.id) ? 'var(--cad-accent-amber)' : 'var(--cad-border)'}`,
              borderRight: `1px solid ${selectedIds.has(rb.id) ? 'var(--cad-accent-amber)' : 'var(--cad-border)'}`,
              borderBottom: `1px solid ${selectedIds.has(rb.id) ? 'var(--cad-accent-amber)' : 'var(--cad-border)'}`,
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                    ...
                    openEntityLabel(
                      ...
                      existing?.config ??
                        defaultLabelStyleConfig({
                          prefix: 'Rotonda',
                          color: defaultColorForKind('rotonda'),
                        }),
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    removeRoundabout(rb.id);
                    useEntityLabelStore.getState().remove(rb.id);
                  }}
                  style={{
                    background: 'none',
                    border: 'none',
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <label style={{ flex: 1, fontSize: '0.6rem', color: 'var(--cad-text-dim)' }}>
                ...
                <input
                  ...
                  value={rb.radiusM}
                  onChange={(e) =>
                    updateRoundabout(rb.id, { radiusM: parseFloat(e.target.value) || rb.radiusM })
                  }
              ...
              <label style={{ flex: 1, fontSize: '0.6rem', color: 'var(--cad-text-dim)' }}>
                ...
                <input
                  ...
                  value={rb.roadWidthM}
                  onChange={(e) =>
                    updateRoundabout(rb.id, {
                      roadWidthM: parseFloat(e.target.value) || rb.roadWidthM,
                    })
                  }
                  onClick={(e) => e.stopPropagation()}
                  className="cad-input cad-input-sm"
