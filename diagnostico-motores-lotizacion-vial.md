...
2. Introducir un lint/test de regresión simple (Vitest) que recorra el código fuente (o, más pragmático, un test unitario por componente) verificando que `getFeatureKind()` es el único punto de lectura de clasificación de feature — al menos cubrir con test los 5 consumidores tocados en Fase 0.
3. Formalizar el **contrato de "manzano lotizado"**: agregar un campo explícito (`lotStatus: 'none' | 'subdivided'` o similar) en vez de inferirlo indirectamente de "¿existe una manzana con este id?" + "¿hay lotes con este lotGroupId?". Este campo es el que va a necesitar la Fase 4 para decidir si re-lotizar automáticamente.
4. Test suite base para `objectModel.ts`, `polygonEngine.ts` (primitivas: `polyArea`, `clipHalfPlane`, `principalAxis`) — hoy no hay cobertura visible de estas funciones puras, que son ideales para tests unitarios rápidos (`vitest` ya está en `package.json`).
...
5. Investigar y, si se confirma, corregir la pérdida de área sistemática descrita en H-LOT-6 (`subdivideManzanoAuto`, filtrado `areaM2 < 0.5` sin redistribución) — agregar un test con un manzano irregular conocido y comparar área total de lotes resultante vs. área original, con una tolerancia definida como criterio de aceptación.
...
1. Suite de tests unitarios (Vitest) para:
   - `polygonEngine.ts`: `polyArea`, `clipHalfPlane`, `clipToStrip`, `principalAxis`, `buildCutPolys` con fixtures geométricos conocidos (cuadrado, L-shape, trapecio).
   - Los tres algoritmos de `subdivisionAlgorithms.ts`/`subdivisionCabeceraCuerpo.ts`: verificar que la suma de áreas de lotes resultantes ≈ área del manzano original (regresión directa de H-LOT-6), y que ningún lote resultante se autointersecta.
   - `roadNetworkEngine.ts`: `offsetPolylineMiter` con casos de ángulo agudo (regresión de H-VIA-1).
   - `ringFillet.ts`: `roundRingReflex` sobre anillos con vértices reflex conocidos.
   - `recomputeManzanosImmediate`: test de integración (mock del worker) cubriendo exactamente el escenario de H-LOT-2/H-LOT-3 ("trazar calle sobre manzano ya lotizado no debe dejar huérfanos").
2. Logging estructurado de errores de worker (hoy `console.error` suelto en varios `catch`) hacia un canal centralizado, con contexto suficiente para reproducir (tipo de operación, tamaño de payload, mensaje de error).
3. Checklist de regresión manual para casos límite: calles con waypoints en zig-zag pronunciado, rotonda triangular con calzada ancha, manzano muy angosto (`NARROW_RATIO`), importación DXF con muchas entidades seguida de generación masiva de lotes.
...
Fase 7 (QA) es transversal: cada fase anterior debería cerrar con al menos
un test de regresión agregado a la suite, no dejarlo todo para el final.
```
