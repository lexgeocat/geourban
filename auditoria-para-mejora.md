...
### Fase 4 — Índice espacial y render a escala — 🟡 EN CURSO (2-ago-2026): 4.2 medida en release → rstar archivado como validado-no-adoptado; restan 4.3 y 4.4
...
**Total estimado:** ~1.5 semanas (baja de ~2.5: la Fase 4.1 quedó **archivada como validado-no-adoptado** el 2-ago-2026 con datos de release — el trabajo de cablear el viewport nativo y la sync incremental se cancela; resta 4.3 LabelPainter y 4.4 capas WebGL).
...
| 4 — Índice espacial + render       | 🟡 En curso (2-ago-2026): bug §5.1 resuelto; **4.2 medida en release → rstar archivado como validado-no-adoptado** (RBush JS gana en todo, ver 4.1/4.2); restan 4.3 (LabelPainter) y 4.4 (capas WebGL) | ~1.5 semanas (baja ~1 semana: se cancela el cableado del viewport nativo) |
| 5 — CRS afín                       | ❌ No iniciada                                   | 1.5-2 semanas                                      |
| 6 — Estrés                         | ❌ No iniciada                                   | 2.5-3 semanas                                      |

**Total restante estimado: ~6.5-7.5 semanas** desde hoy, asumiendo 1-2 ingenieros senior dedicados — **menor que las 7-8 semanas que figuraban ayer**, porque la Fase 3 quedó **cerrada en su totalidad (2-ago-2026, 3.4 medida y criterio cumplido)**, el bug de sincronización del índice (§5.1), que bloqueaba el arranque de la Fase 4, resultó **ya resuelto en código** (2-ago-2026: `load()` explícito en los dos puntos de entrada masivo + telemetría de producción), y la Fase 4 **avanzó el mismo día**: esqueleto `rstar` + harness del benchmark 4.2 armados, y el benchmark **medido en release (2-ago-2026)** — veredicto: **rstar archivado como validado-no-adoptado**, el índice JS RBush se queda como única vía (ver 4.2). El trabajo que sigue es, en orden de impacto: saldar las deudas de la Fase 3 (~2-3 días), 4.3 (caché de `LabelPainter`), 4.4 (presupuesto de capas WebGL) y después Fase 5.
