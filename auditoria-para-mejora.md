...
### Fase 4 — Índice espacial y render a escala — 🟡 EN CURSO (2-ago-2026): 4.1 esqueleto + benchmark 4.2 armados, falta medir y cablear
...
- **4.1 — `rstar` del lado Rust + comando de consulta de viewport** (1.5 semanas → **~1 semana restante, esqueleto hecho el 2-ago-2026**): bulk-load (`RTree::bulk_load`, no inserción incremental — ver §7.4) al hidratar un proyecto grande desde SQLite. **Hecho:** módulo `spatial.rs` en `geourban-geo` (índice id+bbox, bulk_load/insert/remove/search con paridad vs fuerza bruta y sanity 100k — 7 tests, 2-ago-2026); comandos `spatial_index_load`/`spatial_index_query`/`spatial_index_clear` en `geo_bridge.rs` con estado `SpatialIndexState` (registrados en `lib.rs`); cliente `spatialIndexLoadInWorker`/`spatialIndexQueryInWorker`/`spatialIndexClearInWorker` en `geoWorkerClient.ts` (ids viajan como `serde_json::Value`, mismo criterio que `SubdivideManzanoBatchItem`). **Pendiente:** hidratar con bulk-load en `loadProject`/`restoreDrawFeatures` por encima del umbral, decidir el modelo de sincronización incremental en edición (por comando vs. re-hidratar por lote vs. snapshot) y cablear `PostrenderPainter.getVisibleFeatures` a la consulta nativa.
- **4.2 — Umbral de decisión medido** (2-3 días → **~1 día, harness hecho el 2-ago-2026**): usar el instrumental de Fase 0 para fijar el número real de features a partir del cual conviene pagar el costo de un `invoke` en vez de resolver en RBush JS local. No adivinar el umbral — medirlo con el dataset sintético. **Hecho:** benchmark `runSpatialIndexBenchmarkSuite` (`src/geo/debug/spatialIndexBenchmark.ts`) + sección en `DebugPanel` — compara load y query (5 rondas × 3 rects de viewport: centro/esquina/banda) RBush JS vs rstar nativo, con verificación de paridad de ids, en 10k/100k/500k. **Primera corrida (2-ago-2026, build de debug):** paridad ✓ en las 3 escalas (1,549 / 13,590 / 66,272 hits idénticos); load JS 27.9 / 269.8 / 4,196 ms vs nativo 589 / 5,083 / 72,931 ms; query JS 0.27 / 1.71 / 8.11 ms vs nativo 10.3 / 29.6 / 157.2 ms (end-to-end con IPC). **⚠ La corrida NO es válida para decidir:** Rust corrió en build de debug (10-50x más lento que release; rstar con STR packing es especialmente sensible), mientras RBush corre en V8 ya optimizado — "Rust sin optimizar vs JS optimizado". El benchmark ahora separa el costo de búsqueda interna de rstar (`queryMs` del comando, sin IPC) del costo de IPC+serialización. **Decisión pendiente:** re-correr en build release (`npm run tauri build` y correr el instalador/build) y ahí fijar el umbral o archivar el índice nativo como validado-no-adoptado (ver recomendación en §6, Fase 4.2). **Dato nuevo de la corrida de Fase 3.4 (2-ago-2026):** a 500k features el cuello de botella ya no es el undo (resuelto, diff de 508 KB) sino el motor nativo de red vial — `computeRoadNetworkNet` 12,460 ms y `computeManzanos` 7,508 ms en el trazado de una calle que cruza la grilla. Justifica medir el umbral antes de invertir en `rstar`: la pregunta real a esa escala es si el recompute nativo puede pagarse por `invoke` por operación o si necesita batch/delta, más que si la consulta de viewport va por IPC.
...
**Total estimado:** ~2.5 semanas (baja de ~3: el esqueleto de 4.1 —crate, comandos, cliente— y el harness de 4.2 quedaron hechos el 2-ago-2026; resta cablear, medir y decidir el modelo de sync).
...
| 4 — Índice espacial + render       | 🟡 En curso (2-ago-2026): bug §5.1 resuelto en código; esqueleto rstar (crate+comandos+cliente) y harness del benchmark 4.2 hechos; falta medir el umbral, cablear el viewport y la sync incremental | ~2.5 semanas                                   |
| 5 — CRS afín                       | ❌ No iniciada                                   | 1.5-2 semanas                                      |
| 6 — Estrés                         | ❌ No iniciada                                   | 2.5-3 semanas                                      |

**Total restante estimado: ~6.5-7.5 semanas** desde hoy, asumiendo 1-2 ingenieros senior dedicados — **menor que las 7-8 semanas que figuraban ayer**, porque la Fase 3 quedó **cerrada en su totalidad (2-ago-2026, 3.4 medida y criterio cumplido)**, el bug de sincronización del índice (§5.1), que bloqueaba el arranque de la Fase 4, resultó **ya resuelto en código** (2-ago-2026: `load()` explícito en los dos puntos de entrada masivo + telemetría de producción), y la Fase 4 **arrancó el mismo día**: esqueleto `rstar` (crate + comandos + cliente) y harness del benchmark 4.2 listos. El trabajo que sigue es, en orden de impacto: saldar las deudas de la Fase 3 (~2-3 días), correr el benchmark 4.2 y fijar el umbral, cablear la consulta de viewport (4.1) y después Fase 5.
...
| Índice espacial                             | 🟡 En curso (2-ago-2026): bug de resincronización resuelto en código + esqueleto `rstar` nativo listo (crate `spatial.rs`, comandos `spatial_index_*`, cliente) + benchmark 4.2 armado para fijar el umbral; falta cablear el viewport | RBush JS + `rstar` nativo, con causa raíz del bug corregida antes de escalar |
