...
### Fase 4 — Índice espacial y render a escala — 🟡 EN CURSO (2-ago-2026): 4.2 medida en release → rstar archivado como validado-no-adoptado; **4.3 aplicada** (falta medición en uso real); resta 4.4
...
**Total estimado:** ~1 semana (baja de ~2.5: la Fase 4.1 quedó **archivada como validado-no-adoptado** el 2-ago-2026 con datos de release — el trabajo de cablear el viewport nativo y la sync incremental se cancela; la **4.3 quedó aplicada** el mismo día y solo resta medirla en uso real; queda 4.4 capas WebGL).
...
| 4 — Índice espacial + render       | 🟡 En curso (2-ago-2026): bug §5.1 resuelto; **4.2 medida en release → rstar archivado como validado-no-adoptado** (RBush JS gana en todo, ver 4.1/4.2); **4.3 (caché de LabelPainter) aplicada, falta medir en uso real**; resta 4.4 (capas WebGL) | ~1 semana (baja ~2 semanas: se cancela el cableado del viewport nativo y la 4.3 está aplicada) |
| 5 — CRS afín                       | ❌ No iniciada                                   | 1.5-2 semanas                                      |
| 6 — Estrés                         | ❌ No iniciada                                   | 2.5-3 semanas                                      |

**Total restante estimado: ~6-7 semanas** desde hoy, asumiendo 1-2 ingenieros senior dedicados — **menor que las 7-8 semanas que figuraban ayer**, porque la Fase 3 quedó **cerrada en su totalidad (2-ago-2026, 3.4 medida y criterio cumplido)**, el bug de sincronización del índice (§5.1), que bloqueaba el arranque de la Fase 4, resultó **ya resuelto en código** (2-ago-2026: `load()` explícito en los dos puntos de entrada masivo + telemetría de producción), y la Fase 4 **avanzó el mismo día**: esqueleto `rstar` + harness del benchmark 4.2 armados, y el benchmark **medido en release (2-ago-2026)** — veredicto: **rstar archivado como validado-no-adoptado**, el índice JS RBush se queda como única vía (ver 4.2); además **4.3 (caché de `LabelPainter` por firma) quedó aplicada** y solo resta medirla en uso real. El trabajo que sigue es, en orden de impacto: saldar las deudas de la Fase 3 (~2-3 días), medir 4.3, 4.4 (presupuesto de capas WebGL) y después Fase 5.
...
| Índice espacial                             | 🟡 En curso (2-ago-2026): bug de resincronización resuelto en código; **benchmark 4.2 medido en release → `rstar` archivado como validado-no-adoptado** (RBush JS gana en todo, ver Fase 4.2); el esqueleto `rstar` (crate `spatial.rs`, comandos, cliente) se conserva como evidencia; **4.3 (caché de `LabelPainter` por firma) aplicada, falta medir** | RBush JS como única vía (causa raíz del bug corregida); `rstar` solo fuera del IPC, si algún día se necesita batch nativo |
