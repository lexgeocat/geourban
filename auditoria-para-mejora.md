...
Tolerancia: área 1e-3 m², longitudes 1e-3 m, comparando `count`/`totalArea`/`bboxArea`/`remnantCount`/`areas[]`/`frontMs[]`/`depthMs[]`. **Matiz honesto:** el corpus eran anillos de ejemplo representativos, no 20-30 manzanos del dataset sintético con calles/rotondas — esa ampliación fue la deuda de la Fase 6.1, **saldada el 3-ago-2026** con `tests/synthetic_urban_stress.rs` (manzanos irregulares del layout urbano sintético real, ver Fase 6.1). El mecanismo de paridad quedó montado y es ejecutable con cualquier anillo.
...
### Fase 6 — Endurecimiento y pruebas de estrés — ✅ COMPLETA (3-ago-2026): 6.1 corpus sintético urbano, 6.2 memoria nativa, 6.3 fuzzing (cerrada en 2.6), 6.4 carga concurrente

- **6.1 — Ampliar el dataset sintético** — ✅ **RESUELTO EN SU MAYORÍA (3-ago-2026, con un cierre adicional el mismo día).** `src/geo/debug/syntheticDataset.ts` (grilla rectangular) quedó complementado por el **generador de layout urbano avanzado** `syntheticUrbanLayout.ts` (calles con ancho variable, avenidas diagonales que cortan la grilla, rotondas mixtas círculo/polígono, perímetro irregular star-shaped garantizado simple) y su loader/harness `syntheticUrbanBenchmark.ts`, que alimenta el **pipeline real de producción** — `restoreDrawFeatures` (perímetro) → `addStreetWithId`/`addRoundaboutWithId` → `recomputeManzanos()` (GEOS union+difference, Fase 2.3) → pasada incremental con avenida nueva (reconciliación de fragmentos, Fase 2.4) → `subdivideManzanoBatchInWorker` sobre los manzanos irregulares resultantes con chequeo de degenerados (Fase 2.2). UI en `DebugPanel` (sección "Fase 6.1"), tests en `syntheticUrbanLayout.test.ts` (11). **Cierre de la deuda de corpus del crate (3-ago-2026):** la ampliación de cobertura que las Fases 2.2/2.3/2.4 dejaron pendiente quedó saldada con `tests/synthetic_urban_stress.rs` — 8 manzanos irregulares que replican las familias del layout sintético real (triángulo de avenida, trapecio, L de esquina, muesca de rotonda, pentágono/hexágono con jitter, tira angosta) ejercitados con los 3 métodos de subdivisión + `dirPref` + reconciliación de fragmentos (identidad y caso cruzado) con el criterio "0 degenerados" del harness. Todo verde en `cargo test` (3 tests nuevos).
- **6.2 — Profiling de memoria nativo** — ✅ **RESUELTO (3-ago-2026).** Nuevo comando Tauri `process_memory` (`src-tauri/src/process_memory.rs`): RSS / bytes privados / pico de RSS del proceso Rust vía `GetProcessMemoryInfo` (Windows) y `/proc/self/status` (Linux); telemetría TS `nativeMemoryTelemetry.ts` (cache con refresh ≤2s) y sección en `DebugPanel` ("Memoria proceso nativo — RSS Rust") junto al heap JS, con total combinado JS+nativo. El objetivo <2GB con 1M features queda ahora **medible en producción** sin tools externas. **Warning de 1M aplicado:** el botón `1M` del dataset sintético quedó marcado en ámbar con el aviso de riesgo real de OOM/thrash (heap JS al 70% del límite con 500k, corrida 3.4) — el límite no se bajó a propósito: con la medición nativa al lado, el usuario puede decidir con dato, no con miedo.
- **6.3 — Fuzzing de geometría degenerada contra el motor Rust activo** — ✅ **CUMPLIDA (cerrada como parte de la Fase 2.6, 1-ago-2026):** fuzz TS (236 casos) + fuzz Rust con timeout (`fuzz_degenerate_geometry.rs`, 2 tests, 0 cuelgues/fallos en la corrida del 3-ago-2026). Sin trabajo restante.
- **6.4 — Pruebas de carga concurrente** — ✅ **RESUELTO (3-ago-2026).** Harness `concurrencyStressBenchmark.ts` (puro, sin stores — mismo criterio que `spatialIndexBenchmark`): rings de manzano irregulares deterministas (star-shaped, garantía de simplicidad), fase serial vs. paralela (`Promise.all`) de `subdivideManzanoBatch` por chunks, fase mixta con `computeRoadNetworkNet` ∥ `matchFragmentsBatch`, y **stall del event loop** medido con muestreo rAF (reposo vs. bajo carga). Output: `parallelSpeedup` (serial/paralela; >1 = el runtime async de Tauri realmente paraleliza) y `stallDegradationRatio` (≈1 = la interacción no percibe la carga). UI en `DebugPanel` ("Fase 6.4 — Carga concurrente"), suite estándar 32/64/128 manzanos, tests del generador en vitest (5). Criterio de éxito: 0 lotes degenerados en todas las escalas.

**Total estimado: cerrado el 3-ago-2026 — las cuatro sub-fases de la Fase 6 quedaron completas** (6.1 con su cierre de corpus del crate, 6.2 con medición nativa + warning 1M, 6.3 ya cerrada en 2.6, 6.4 con harness de concurrencia).
...
| 6 — Estrés                         | ✅ **Completa (3-ago-2026):** 6.1 corpus sintético urbano + cierre de corpus del crate (`synthetic_urban_stress.rs`), 6.2 `process_memory` (RSS nativo) + warning 1M, 6.3 fuzzing (ya cerrada en 2.6), 6.4 harness de carga concurrente con stall del event loop | 0 (terminada) |

**Total restante estimado: ~2-3 días** (solo las deudas de la Fase 3: telemetría muerta y tests de `composeStructuralDiffs`/`CommandStack`) — la Fase 6 quedó **cerrada en su totalidad el 3-ago-2026** (6.1 corpus sintético urbano + cierre de corpus del crate, 6.2 `process_memory` con RSS nativo + warning 1M, 6.3 fuzzing ya cerrada en 2.6, 6.4 harness de carga concurrente con stall del event loop) y la Fase 5 había cerrado el mismo día con `TiledAffineCache` (error max 300.20mm→0.36mm, degraded 4004→0). La hoja de ruta queda en: saldar las deudas menores de la Fase 3 (~2-3 días).
...
| Memoria con dataset de 1M features          | ✅ **Medible desde el panel de debug (3-ago-2026, Fase 6.2):** comando `process_memory` expone RSS/privados/pico del proceso Rust junto al heap JS ("Memoria proceso nativo" en `DebugPanel`); el botón de 1M del dataset sintético lleva warning de OOM (heap JS ~70% del límite con 500k) | < 2GB confirmado con profiler nativo (medición en `DebugPanel`, sin tools externas) |
...
| Cobertura de tests de paridad JS↔Rust       | ✅ 4 de 4 módulos del motor con paridad automática (subdivisión auto/exact/modo2, fragmentos, computeManzanos) + fuzzing completo (TS 236 casos + Rust con timeout) + tests unitarios en los 9 archivos del crate | ✅ **Corpus ampliado (3-ago-2026, Fase 6.1):** `synthetic_urban_stress.rs` ejercita manzanos irregulares del layout urbano real (triángulos, trapecios, L, muescas de rotonda, jitter) con los 3 métodos + `dirPref` + reconciliación — cumplido |

---

Esta sigue siendo una hoja de ruta, no una promesa — la diferencia con la versión anterior de este documento es que ahora cada casilla de "completado" está respaldada por una lectura real del código y por tests verdes, no por la expectativa de que el plan se ejecutó tal como se escribió. El cierre de la Fase 2 (1-ago-2026) y el de la Fase 3 (2-ago-2026) dejan la hoja de ruta en su punto más simple desde que existe: **las tres piernas de la auditoría — portar el algoritmo (2.0-2.4), conectarlo al producto (2.5) y probarlo en producción y borrar lo viejo (2.6 fuzzing + 2.7 limpieza) — están hechas, el undo/redo estructural que era el otro cuello de botella crítico de §2.1 está completo y medido (3.0-3.4): diffs proporcionales al cambio, con la medición confirmando el criterio (ratio undo/snapshot 0.47% @ 500k y decreciente con n), el bug de sincronización del índice espacial (§5.1), el hallazgo más urgente de las revisiones anteriores, resultó ya resuelto en código (carga explícita del índice en ambos puntos de entrada masivo + telemetría de producción), la Fase 4 cerró con `rstar` archivado como validado-no-adoptado (2-ago-2026), la Fase 5 cerró con `TiledAffineCache` (3-ago-2026) y la Fase 6 cerró el mismo día: corpus sintético urbano con cierre de cobertura del crate (6.1), profiling de memoria nativa con warning de 1M (6.2), fuzzing ya cerrado en 2.6 (6.3) y harness de carga concurrente con medición de stall del event loop (6.4).** Lo que queda, en orden de impacto: saldar las deudas menores de la Fase 3 (~2-3 días: telemetría muerta y tests de `composeStructuralDiffs`/`CommandStack`).
